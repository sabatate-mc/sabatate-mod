    ...
    override fun useOnBlock(context: ItemUsageContext?): ActionResult {
        ...
        if (blockTop.block == Blocks.STONE && blockMiddle.block == Blocks.STONE && blockBottom.block == Blocks.STONE) {
            if (!world.isClient) {
                ...
                world.setBlockState(pos, ModBlocks.SABATATE_STATUE_TOP.defaultState.with(SabatateStatueTop.FACING, direction).with(SabatateStatueTop.TORCH_OFF, true))
                world.setBlockState(pos.down(1), ModBlocks.SABATATE_STATUE_MIDDLE.defaultState.with(SabatateStatueMiddle.FACING, direction).with(SabatateStatueMiddle.TORCH_OFF, true))
                world.setBlockState(pos.down(2), ModBlocks.SABATATE_STATUE_MIDDLE.defaultState.with(SabatateStatueMiddle.FACING, direction).with(SabatateStatueMiddle.TORCH_OFF, true))
